...
if (Meteor.isClient) {
	...
	Template.trades.list = function() {
		return Trades.find({}, {sort: {time: -1}}).fetch();
	};
	...
	Meteor.startup(function() {
		...
		function displayGraphExample(id, width, height, interpolation, animate, updateDelay, transitionDelay) {
			...
			function getData() {
				var trades = Trades.find({}, {sort: {time: -1}}).fetch().slice(0,10);
				data = [];
...
//				var data = [3, 6, 2, 7, 5, 2, 1, 3, 8, 9, 2, 5, 9, 3, 6, 3, 6, 2, 7, 5, 2, 1, 3, 8, 9, 2, 5, 9, 2, 7, 5, 2, 1, 3, 8, 9, 2, 5, 9, 3, 6, 2, 7, 5, 2, 1, 3, 8, 9, 2, 9];
			...
			displayGraphExample("#graph", 1000, 500, "basis", true, 1000, 1000);
/*
